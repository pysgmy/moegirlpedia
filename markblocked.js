 ...
 * 取自[[zhwiki:MediaWiki:Gadget-markblocked.js]]，oldid=65734356。
 * 已萌百化
...
function markBlocked(container) {
	...
	if (!window.mbTooltipPartial) {
		...
		i18n['zh-cn'] = i18n['zh-hans'];
		i18n['zh-tw'] = i18n['zh-hant'];
		mbTooltipPartial = i18n[mw.config.get('wgUserLanguage')] || i18n.zh;
	}
	...
	if (window.markblocked_contributions === undefined) {
		window.markblocked_contributions = 'Special:(?:Contribs|Contributions|用户贡献|用戶貢獻|使用者贡献|使用者貢獻)';
	}
	...
	contentLinks.each(function (i, lnk) {
		if ($(lnk).hasClass('mw-changeslist-date') || $(lnk).parent('span').hasClass('mw-history-undo') || $(lnk).parent('span').hasClass('mw-rollback-link') || $(lnk).parent('span').hasClass('comment')) {
			return;
		}
		url = $(lnk).attr('href');
		if (!url) {
		...
		if (ma = scriptRX.exec(url)) {
			pgTitle = ma[1];
		} else if (ma = articleRX.exec(url)) {
			pgTitle = ma[1];
		} else {
			return;
		}
